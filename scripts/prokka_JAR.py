...
Genomes_file = open( "/mnt/gs18/scratch/groups/manninglab/cattlegenomes.txt" ) #Text file with all the genomes to be used in this pipeline. File is used instead of a directory in order to easily edit which genomes are utilized
...
for i in Genomes_pair: #Here we are stripping tabs and spaces from file names 
	...
	os.system("prokka  --outdir /mnt/gs18/scratch/groups/manninglab/cattle_prokka_results/" + RefGenome + " --prefix "  +  RefGenome   + "   /mnt/gs18/scratch/groups/manninglab/cattle_genomes/" + RefGenome + ".fasta --compliant") #Here we are using Prokka to annotate genomes see https://github.com/tseemann/prokka
