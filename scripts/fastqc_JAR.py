...
Genomes_file = open( "/mnt/gs18/scratch/groups/manninglab/ecoli_genomes.txt" ) #Text file with all the genomes to be used in this pipeline. File is used instead of a directory in order to easily edit which genomes are utilized
...
for i in Genomes_pair: #Here we are stripping tabs and spaces from file names 
	...
	os.system("fastqc [path to read s1 ] s1_* [path to read s2 ] s2_* --outdir= [Path to results folder] +RefGenome+ "/fastqc")
...
#Fastqc is the call and the commands. my file directory has the genome name / with s1_pe file names * means ignore the rest or all files with that s1_ in the name we provide the s1 and s2 and the outdirectory file   I put the output in files that have the name first and fastqc